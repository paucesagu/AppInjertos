    ...
    request(
      ...
      controller.reentrenar = async (req, res) => {
        try{
            ...
            await connection.query('SELECT * FROM reentrenamientos;', (err, result) => {
                if(result.length===0){ //no existe ningun reentrenamiento
                    indice = 0;
                  
                }else{
                  //si existe un reentrenamiento devolvemos el mayor indice
                  console.log("va a hacer la conexion")
                  var result = await connection.query('SELECT ultima_instancia FROM reentrenamientos where ultima_instancia = (select max(ultima_instancia) from reentrenamientos)');
                  console.log(result)
